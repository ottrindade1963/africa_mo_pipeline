...
class FoldFeatureEngineer(BaseEstimator, TransformerMixin):
    ...
    def _add_interactions(self, df):
        pca_lag = "wgi_pca1_lag1" if "wgi_pca1_lag1" in df.columns else None
        if pca_lag is None:
            return df
        ...
        for eco in eco_vars:
            ...
            if eco_col not in df.columns:
                continue
            name  = f"inter_pca1_{eco.split('_')[0]}"
            s_pca = df[pca_lag].std()  or 1.0
            s_eco = df[eco_col].std()  or 1.0
            df[name] = (df[pca_lag] / s_pca) * (df[eco_col] / s_eco)
        return df
