...
def _run_explainability(spec_datasets: dict) -> None:
    ...
    for spec_name, df in spec_datasets.items():
        ...
        X_test    = df[df["year"].isin(test_yr)][feat_cols].fillna(0)
        ...
        # KernelExplainer for SARIMAX on the main spec
        if spec_name == "WDI_plus_PCA1":
            for mod_name in ["SARIMAX", "LSTM", "Bayes_Partial"]:
                ...
                label = f"{spec_name}_{mod_name}"
                try:
                    shap_kernel_analysis(model, X_all,
                                         X_all.sample(min(200, len(X_all)), random_state=42),
                                         label, exp_dir)
                except Exception as exc:
...
def run_pipeline():
    ...
    best_rmse = float(df_wf["RMSE"].dropna().min()) if not df_wf.empty else np.nan
    best_row  = df_wf.loc[df_wf["RMSE"].idxmin()] if not df_wf.empty else {}

    run_all_reports(
        results_csv=wf_path,
        hp_csv=hp_path,
        sarimax_coef_csv=os.path.join(paths.REPORTS_DIR,
                                       "sarimax_WDI_plus_PCA1_coef.csv"),
        ablation_csv=os.path.join(abl_dir, "ablation_results.csv"),
        ablation_dm_csv=os.path.join(abl_dir, "ablation_dm_tests.csv"),
        summary_kv={
            "best_RMSE":  best_rmse,
            "best_model": getattr(best_row, "model", "—"),
            "best_spec":  getattr(best_row, "spec",  "—"),
